...
async function getRelativeStackFilePath(string, userAgent, rootDir, stackLocationRegExp, sourceMapFunction) {
    ...
    if (match) {
        ...
        const fullFilePath = browserPath.startsWith(rootDir)
            ? browserPath
            : path_1.default.join(rootDir, toFilePath_1.toFilePath(browserPath));
        const fullBrowserPath = `${browserPath}${browserParams}`;
        const sourceMapResult = await sourceMapFunction(fullBrowserPath, fullFilePath, userAgent, Number(line), Number(column));
